...
pub fn runtime_from(candidates: Vec<(PathBuf, &'static str)>) -> Runtime {
    if candidates.is_empty() {
        return Runtime {
            ...
            origin: None,
            probe: None,
            detail: "The llama.cpp runtime is not installed. Reinstall ShadowCode (the AppImage and .deb include it) or build it with scripts/build-llama.cpp.sh. Models are never downloaded automatically.".into(),
        };
    }

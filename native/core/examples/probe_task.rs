...
async fn main() -> Result<()> {
    ...
    let second=engine.start(StartRequest{workspace:project.clone(),task:"Read src/lib.rs again and tell me what add(-2, 1) now returns. Do not change files or run commands.".into(),session_id:Some(first.session_id.clone()),model:None,mode:"review".into(),queue:false,images: Vec::new(),web:false}).await?;
    let continued =

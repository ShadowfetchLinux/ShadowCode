...
async fn main() -> anyhow::Result<()> {
    ...
    let probe =
        shadowcode_core::cli_agent::codex_probe::probe(&binary, None, Duration::from_secs(30))
            .await?;
    println!(
        "auth_mode={} logged_in={} subscription={} plan={:?}",
        probe.auth_mode(),
        probe.logged_in(),
        probe.subscription_login(),
        probe.plan_type()
    );
    if let Some(limits) = &probe.rate_limits {
        ...
        println!(
            ...
            snap["primary"]["usedPercent"],
            snap["primary"]["windowDurationMins"],
            snap["primary"]["resetsAt"],
            snap["credits"]
        );
        if let Some(pools) = limits["rateLimitsByLimitId"].as_object() {
            for (id, pool) in pools {
                println!(
                    "pool {id}: name={:?} slug={:?} used={}%",
                    pool["limitName"], pool["normalModelSlug"], pool["primary"]["usedPercent"]
                );
            }
        }
    }
    for model in shadowcode_core::cli_agent::codex_probe::models_from_list(&probe.models) {
        println!(
            ...
            model.id, model.label, model.is_default, model.vision
        );
    }

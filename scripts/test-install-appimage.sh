...
APPIMAGE="$SCRATCH/release/ShadowCode_0.20.0_amd64.AppImage"
printf '%s\n' \
  '#!/usr/bin/env bash' \
  'set -euo pipefail' \
  'if [[ "$1" == "--appimage-extract-and-run" && "$2" == "--version" ]]; then' \
  '  printf "ShadowCode 0.20.0\\n"' \
  '  exit 0' \
  'fi' \
  'exit 1' > "$APPIMAGE"
chmod +x "$APPIMAGE"
...
test -f "$HOME/Applications/ShadowCode-0.20.0-x86_64.AppImage"
test "$(readlink "$HOME/Applications/ShadowCode.AppImage")" = ShadowCode-0.20.0-x86_64.AppImage
...
"$HOME/.local/bin/shadow" --version | grep -Fx 'ShadowCode 0.20.0'

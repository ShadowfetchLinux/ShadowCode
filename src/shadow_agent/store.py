...
class Store:
    ...
    def create_session(self, workspace: str, model_id: str | None = None) -> str:
        sid = uuid.uuid4().hex
        now = time.time()
        self._conn.execute(
            "INSERT INTO sessions(id, workspace, created_at, updated_at, model_id, status) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (sid, workspace, now, now, model_id, "active"),
        )
        self._conn.commit()
        return sid

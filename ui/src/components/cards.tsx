...
const MUTATING = new Set(["write_file", "edit_file", "delete_file", "move_file", "apply_patch"]);
...
export function OpCard({
  ...
  const canRewind = Boolean(item.taskId) && isMutatingTool(item.tool) && item.ok !== false && !item.live;
  const canDiff = Boolean(item.path) && isMutatingTool(item.tool) && item.ok !== false && !item.live;
  return (
    <div className={`op-card ${item.ok === false ? "bad" : ""} ${open ? "open" : ""} ${item.live ? "live" : ""}`}>
          ...
          {(canRewind || canDiff) && (
            <div className="op-actions">
              {canDiff && item.path && onReviewDiff && (
                <button type="button" className="mini" onClick={() => onReviewDiff(item.path as string)}>Review diff</button>
              )}
              {canRewind && item.taskId && onRewind && (
                <button type="button" className="mini" title="Undo every file change made by this task" onClick={() => onRewind(item.taskId as string)}>↶ Rewind</button>
              )}
            </div>
          )}
        </div>
...
export function ApprovalCard({ approval, onDecide }: { approval: Approval; onDecide: (id: string, decision: "approve" | "deny") => void }) {
  return (
    <div className="approval" data-approval-id={approval.id}>
      ...
      {approval.command && approval.reason && <p className="hint">{approval.reason}</p>}
      <div className="row approval-actions">
        <button type="button" className="ghost" onClick={() => onDecide(approval.id, "deny")}>Cancel <span className="kbd">Esc</span></button>
        <button type="button" className="primary" onClick={() => onDecide(approval.id, "approve")}>Allow <span className="kbd-hint">↵</span></button>
      </div>
    </div>
  );
}
    ...
    return (
      <div className="tool-card">
        ...
        {card.diff.split("\n").slice(0, 200).map((line, i) => (
          <div key={i} className={`diff-line ${line.startsWith("+") ? "diff-add" : line.startsWith("-") ? "diff-del" : "diff-ctx"}`}>{line}</div>
        ))}
      </div>
    );
  }
    ...
    return (
      <div className="tool-card">
        ...
        {card.items.map((item, i) => (
          <div key={i} className="status-row"><span>{item.label}</span><code>{item.value}</code></div>
        ))}
      </div>
    );
  }

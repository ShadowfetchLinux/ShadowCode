...
const steps = (events: EventRow[], pending = 0) =>
  deriveSteps(replay(events).activity.t1, pending).map((s) => [s.label, s.state]);

describe("activity timeline from real events", () => {
  it("native tools: read, write, pytest, completion", () => {
    const events = [
      ev("agent.started", { task: "Fix it" }),
      ev("tool.started", { tool: "read_file", call_id: "a", arguments: { path: "app.py" } }),
      ev("tool.completed", { tool: "read_file", call_id: "a", success: true, output_preview: "def add" }),
      ev("tool.started", { tool: "write_file", call_id: "b", arguments: { path: "app.py" } }),
      ev("tool.completed", { tool: "write_file", call_id: "b", success: true, arguments: { path: "app.py" } }),
      ev("checkpoint.updated", { paths: ["app.py"], changes: 1 }),
      ev("tool.started", { tool: "exec", call_id: "c", arguments: { command: "python -m pytest -q" } }),
    ];
    ...
    const done = [
      ...events,
      ev("tool.completed", { tool: "exec", call_id: "c", success: true, output: { exit_code: 0 } }),
      ev("verification.summary", {
        status: "last_command_succeeded",
        commands: [{ command: "python -m pytest -q", success: true, exit_code: 0 }],
      }),
      ev("agent.completed", { summary: "Fixed", success: true }),
    ];
    const state = replay(done);
    expect(deriveSteps(state.activity.t1).map((s) => [s.label, s.state])).toEqual([
      ["Reading project", "done"],
      ["Editing files", "done"],
      ["Running tests", "done"],
      ["Finished", "done"],
    ...
    expect(reading.calls[0]).toMatchObject({ tool: "read_file", output: "def add", ok: true });
  });

  it("vendor tools: Codex, Cursor ACP and Claude names", () => {
    const events = [
      ev("agent.started", { task: "Vendor" }),
      ev("tool.started", { tool: "cursor.read", call_id: "r", arguments: { title: "Read app.ts" } }),
      ev("tool.completed", { tool: "cursor.read", call_id: "r", success: true }),
      ev("tool.started", { tool: "codex.file_change", call_id: "f", arguments: { paths: ["src/a.rs"] } }),
      ev("tool.completed", { tool: "codex.file_change", call_id: "f", success: true }),
      ev("files.changed", { paths: ["src/a.rs"], vendor: "cli-codex" }),
      ev("tool.started", { tool: "codex.command_execution", call_id: "x", arguments: { command: "cargo test --lib" } }),
      ev("tool.completed", { tool: "codex.command_execution", call_id: "x", success: false }),
      ev("tool.started", { tool: "Bash", call_id: "b", arguments: { input: { command: "ls -la" } } }),
  ...
  it("approvals, web sources, handoff and limit events", () => {
    const events = [
      ...
      ev("tool.started", { tool: "web_fetch", call_id: "w", arguments: { url: "https://docs.rs" } }),
      ev("web.source", { url: "https://docs.rs/x", final_url: "https://docs.rs/x/1", title: "x docs", status: 200 }),
      ev("tool.completed", {
        tool: "web_fetch",
        call_id: "w",
        success: true,
      ...
      ev("approval.requested", { id: "ap1", tool: "exec", command: "rm -rf build" }),
    ];
    expect(steps(events)).toEqual([
    ...
    const state = replay([
      ...
      ev("agent.handoff", { from: "local:llamacpp", to: "cli:cursor", excerpt_chars: 1200 }),
      ev("limit.reached", { vendor: "cli-codex", usage: { state: "limit_reached", label: "Limit" } }),
    ]);
    expect(state.activity.t1.sources.map((s) => s.url)).toEqual([
      "https://docs.rs/x",
      "https://docs.rs/y",
    ]);
    expect(deriveSteps(state.activity.t1).find((s) => s.id === "waiting")?.state).toBe("done");
    ...
    expect(replay([...events, ev("limit.reached", { vendor: "codex" }), ev("agent.started", { task: "again" }, "t2")]).limit).toBeUndefined();
  });
